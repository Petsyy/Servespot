...
export default function VolunteerNotifications() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const res = await getVolunteerNotifications(volunteerId);
        if (!mounted) return;
  ...
  useEffect(() => {
    ...
    const onNew = (notif) => {
      if (
        notif?.userModel === "Volunteer" &&
        String(notif?.user) === String(volunteerId)
      ) {
  ...
  // =============================
  // Filter by System / Opportunity / All
  // =============================
  ...
  const markAllAsRead = async () => {
    try {
      if (!volunteerId) return;
      await markVolunteerNotificationsRead(volunteerId);
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <VolunteerNavbar onToggleSidebar={() => setIsSidebarOpen(true)} />
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-gray-600 text-sm">
                  Email & In-App notifications provide updates on opportunities,
                  reminders, and account actions.
                </p>
              </div>
